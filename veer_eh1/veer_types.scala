...
// A nested Scala package becomes a dedicated HDL package named relative to the top design's
// namespace, so these emit as SystemVerilog `veer_types::<type>` and VHDL `veer_types` — the
// baseline's own `import veer_types::*`.
package veer_types {
